...
def test_full_pipeline(test_workspace):
    ...
    profiles_file = workspace / "outputs" / "crispr_trimmed_public" / "profiles_trimmed.parquet"
    ...
    expected_df = pd.read_parquet(Path(__file__).parent / "fixtures" / "outputs" / "crispr_trimmed_public" / "profiles_trimmed.parquet")
                ...
                for col in diff_cols[:5]:  # Show first 5 differing columns
                    ...
                    if mismatch_mask.any():
                        ...
                        for idx in mismatched_rows:
                            print(f"Row {idx}: {actual_df.loc[idx, col]} vs {expected_df.loc[idx, col]}")
